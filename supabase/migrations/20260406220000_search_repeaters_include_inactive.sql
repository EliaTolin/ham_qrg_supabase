-- Add optional p_include_inactive parameter to search_repeaters.
-- Default false → existing clients (Flutter app) keep seeing only active repeaters.
-- Dashboard passes true to also show inactive repeaters.
...
begin
  ...
  with matched as (
    ...
    where
      (r.is_active = true or p_include_inactive = true)
      and (
